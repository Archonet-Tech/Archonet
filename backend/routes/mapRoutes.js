...
// router.post('/polygon', async (req, res) => {
//   const { coordinates, area, sideLengths, trees = [] } = req.body;
//   console.log('Received polygon data:', req.body);
...
// Save location search route
router.post('/search-location', async (req, res) => {
  const { query, place_name, lng, lat } = req.body;

  console.log('Received search location data:', req.body);

  if (!query || !place_name || lng == null || lat == null) {
  ...
  try {
    const { data, error } = await supabase
      .from('search_locations')
      .insert([{ query, place_name, coordinates: { lng, lat } }])
      .select()
      .single();
    ...
    console.log('Search location saved successfully:', data);
    res.status(200).json({ message: 'Location saved successfully', data });
  } catch (error) {
    console.error('Server error saving search location:', error);
